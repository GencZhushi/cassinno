...
import { 
  playBigBassBonanza, 
  SYMBOL_EMOJIS,
} from "@/lib/games/big-bass-bonanza";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const gameConfig = await prisma.gameConfig.findUnique({
      where: { gameType: "BIG_BASS_BONANZA" },
    });
    ...
    if (!isFreeSpinMode && betAmount > 0) {
      const betResult = await placeBet(session.userId, betAmount, "BIG_BASS_BONANZA");
    ...
    const fairnessSeed = await getOrCreateFairnessSeed(session.userId, "BIG_BASS_BONANZA");
    ...
    // Play the game
    const result = playBigBassBonanza(
      betAmount,
      randomFn,
      isFreeSpinMode,
      currentMultiplier,
    ...
    if (result.totalWin > 0) {
      await creditWinnings(session.userId, result.totalWin, "BIG_BASS_BONANZA");
    }
    ...
    return NextResponse.json({
      ...
      moneyFish: result.moneyFish,
      fishermanPositions: result.fishermanPositions,
      collectedValue: result.collectedValue,
      currentMultiplier: result.currentMultiplier,
      fishermenCollected: result.fishermenCollected,
      newBalance: newBalance.toString(),
    });
  } catch (error) {
    console.error("Big Bass Bonanza error:", error);

...
import { 
  playGatesOfOlympus, 
  SYMBOL_EMOJIS,
} from "@/lib/games/gates-of-olympus";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const gameConfig = await prisma.gameConfig.findUnique({
      where: { gameType: "GATES_OF_OLYMPUS" },
    });
    ...
    if (!isFreeSpinMode && betAmount > 0) {
      const betResult = await placeBet(session.userId, betAmount, "GATES_OF_OLYMPUS");
    ...
    const fairnessSeed = await getOrCreateFairnessSeed(session.userId, "GATES_OF_OLYMPUS");
    ...
    // Play the game
    const result = playGatesOfOlympus(
      betAmount,
      randomFn,
      isFreeSpinMode,
      currentMultiplier
    ...
    if (result.totalWin > 0) {
      await creditWinnings(session.userId, result.totalWin, "GATES_OF_OLYMPUS");
    }
    ...
    return NextResponse.json({
      ...
      zeusMultipliers: result.zeusMultipliers,
      newBalance: newBalance.toString(),
    });
  } catch (error) {
    console.error("Gates of Olympus error:", error);

import { NextRequest, NextResponse } from "next/server";
import { getSession } from "@/lib/auth";
import { getBalance } from "@/lib/wallet";
import prisma from "@/lib/prisma";

function calculateMultiplier(revealed: number, mineCount: number): number {
  if (revealed === 0) return 1;
  
  const n = 25;
  const m = mineCount;
  const safeTiles = n - m;
  
  // Probability of surviving k reveals = product of (safeTiles-i)/(n-i) for i=0 to k-1
  let probability = 1;
  for (let i = 0; i < revealed; i++) {
    probability *= (safeTiles - i) / (n - i);
  }
  
  // Multiplier = 1/probability * (1 - houseEdge)
  const multiplier = (1 / probability) * 0.99;
  return Math.floor(multiplier * 100) / 100;
}

export async function POST(request: NextRequest) {
  try {
    const session = await getSession();
    if (!session) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    const body = await request.json();
    const { sessionId, index } = body;

    if (!sessionId || index === undefined || index < 0 || index > 24) {
      return NextResponse.json({ error: "Invalid request" }, { status: 400 });
    }

    const gameSession = await prisma.gameSession.findUnique({
      where: { id: sessionId },
    });

    if (!gameSession || gameSession.userId !== session.userId) {
      return NextResponse.json({ error: "Game not found" }, { status: 404 });
    }

    if (gameSession.status !== "ACTIVE") {
      return NextResponse.json({ error: "Game already finished" }, { status: 400 });
    }

    const state = JSON.parse(gameSession.stateJson);
    const { minePositions, mineCount, revealed, betAmount } = state;

    if (revealed.includes(index)) {
      return NextResponse.json({ error: "Already revealed" }, { status: 400 });
    }

    const isMine = minePositions.includes(index);
    revealed.push(index);

    let gameOver = false;
    let won = false;
    let multiplier = 1;
    let currentWin = 0;

    if (isMine) {
      gameOver = true;
      won = false;
      
      await prisma.gameSession.update({
        where: { id: sessionId },
        data: {
          status: "COMPLETED",
          winAmount: BigInt(0),
          stateJson: JSON.stringify({ ...state, revealed }),
        },
      });
    } else {
      const gemsRevealed = revealed.length;
      multiplier = calculateMultiplier(gemsRevealed, mineCount);
      currentWin = Math.floor(betAmount * multiplier);

      // Check if all safe spots revealed
      if (gemsRevealed === 25 - mineCount) {
        gameOver = true;
        won = true;
      }

      await prisma.gameSession.update({
        where: { id: sessionId },
        data: {
          stateJson: JSON.stringify({ ...state, revealed }),
        },
      });
    }

    const newBalance = await getBalance(session.userId);

    return NextResponse.json({
      isMine,
      multiplier,
      currentWin,
      gameOver,
      won,
      minePositions: isMine ? minePositions : undefined,
      newBalance: newBalance.toString(),
    });
  } catch (error) {
    console.error("Mines reveal error:", error);
    return NextResponse.json({ error: "Game error" }, { status: 500 });
  }
}
